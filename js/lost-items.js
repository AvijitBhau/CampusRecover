...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (params.get('category') && categorySel) categorySel.value = params.get('category');
      ...
      if (filtered.length === 0) {
        grid.innerHTML = `
          <div class="empty-state" style="grid-column:1/-1;">
            ...
            <div class="empty-state-desc">Try adjusting your search or filters. You can also report a new lost item.</div>
            <a href="report-lost.html" class="btn btn-primary">📝 Report Lost Item</a>
          </div>`;
        return;
      }

      grid.innerHTML = filtered.map(item => renderItemCard(item, 'lost')).join('');
      bindCardEvents(grid, 'lost');
